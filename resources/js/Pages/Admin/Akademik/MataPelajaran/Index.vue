...
const submitEdit = () => {
    editForm.put(route('admin.web.mata-pelajaran.update', editTarget.value.hashid), {
        onSuccess: () => {
...
const confirmDelete = () => {
    ...
    router.delete(route('admin.web.mata-pelajaran.destroy', deleteTarget.value.hashid), {
        onSuccess: () => {
...
const handleSearch = () => {
    router.get(route('admin.web.mata-pelajaran.index'), {
        search: search.value,
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100 text-sm">
                            ...
                            >
                                ...
                                <td class="p-4">
                                    <div v-if="mapel.gurus && mapel.gurus.length > 0" class="flex flex-wrap gap-1.5">
                                        <span
                                            v-for="guru in mapel.gurus"
                                            :key="guru.id"
                                            class="inline-flex items-center gap-1 px-2.5 py-1 bg-sky-50 text-sky-700 font-bold text-xs rounded-lg border border-sky-100"
                                        >
                                            <Users class="w-3 h-3" />
                                            {{ guru.nama }}
                                        </span>
            ...
            <div class="bg-white w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden text-center">
                <div class="p-8">
                    ...
                    <p class="text-slate-400 text-xs mb-6">
                        Data ini akan dihapus secara permanen (soft delete). Guru yang terdaftar sebagai pengampu tidak akan terpengaruh.
                    </p>
