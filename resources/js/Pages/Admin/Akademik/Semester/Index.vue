...
import { Layers, Plus, Search, Pencil, Trash2, X, Check, CheckCircle, XCircle } from 'lucide-vue-next';
...
const handleSearch = () => {
    router.get(route('admin.web.semester.index'), {
        search: search.value,
...
const submitForm = () => {
    if (editTarget.value) {
        form.put(route('admin.web.semester.update', editTarget.value.hashid), {
            onSuccess: () => { showModal.value = false; form.reset(); },
        });
    } else {
        form.post(route('admin.web.semester.store'), {
            onSuccess: () => { showModal.value = false; form.reset(); },
        });
    }
};
...
const hapus = (item) => {
    if (confirm(`Hapus semester "${item.nama}"?`)) {
        router.delete(route('admin.web.semester.destroy', item.hashid));
    }
};
</script>
        ...
        <div class="max-w-7xl mx-auto space-y-6">
            <div class="flex flex-col sm:flex-row justify-between gap-4 items-start sm:items-center">
                <div>
                    ...
                    <p class="text-slate-500 font-medium mt-1">Kelola semester dan tentukan semester aktif.</p>
                </div>
                <button @click="openCreate" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-xl flex items-center gap-2 transition-colors shadow-lg shadow-indigo-200">
                    <Plus class="w-5 h-5" /> Tambah Semester
                </button>
            </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50/50 border-b border-slate-200 text-xs font-bold text-slate-500 uppercase tracking-wider">
                                <th class="p-4 pl-6">Nama Semester</th>
                                <th class="p-4">Status</th>
                                <th class="p-4 pr-6 text-right">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100 text-sm">
                            <tr v-for="item in semesterList.data" :key="item.id" class="hover:bg-slate-50/50 transition-colors">
                                <td class="p-4 pl-6">
                            ...
                            <tr v-if="semesterList.data.length === 0">
                                <td colspan="3" class="p-12 text-center">
                                    <div class="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center mx-auto mb-3 text-slate-400">
                                        <Layers class="w-6 h-6" />
                                    </div>
                                    <p class="font-bold text-slate-700">Belum ada semester</p>
                                    <p class="text-slate-500 text-xs mt-1">Tambahkan semester baru untuk memulai.</p>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <Pagination :data="semesterList" />
            </div>
        </div>
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            <div class="bg-white w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden">
                <div class="flex items-center justify-between p-6 border-b border-slate-100">
                    <h2 class="text-xl font-black text-slate-900">{{ editTarget ? 'Edit Semester' : 'Tambah Semester Baru' }}</h2>
                ...
                <form @submit.prevent="submitForm" class="p-6 space-y-4">
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">Nama Semester</label>
                    ...
                    <div>
                        <label class="flex items-center gap-3 cursor-pointer">
                            ...
                            <span class="text-sm font-bold text-slate-700">Jadikan semester aktif</span>
                        </label>
